...
export default async function ClientProfilePage({ params }: Props) {
  ...
  const currentUser = session ? { role: session.role as string, id: session.userId as string } : null;

  if (id === 'new') {
    return <ClientProfileDetail clientId="new" currentUser={currentUser} />;
  }

  const payload = await getClientProfilePageData(id);
  if (!payload) {
    return <ClientProfileDetail clientId={id} currentUser={currentUser} />;
  }
  ...
  return (
    <>
      ...
      <ClientProfileDetail
        ...
        initialRegularClients={
          payload.regularClientsData?.filter((c): c is NonNullable<typeof c> => c != null) ?? []
        }

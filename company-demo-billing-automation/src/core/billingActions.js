...
async function executeBillingOnPage(page, requestData) {
    ...
    const skipBillingSubmit = /^(1|true|yes|on)$/i.test(String(process.env.DEMO_SKIP_BILLING_SUBMIT || '').trim());

    try {
        const result = await page.evaluate(async (arg) => {
            const { data, sel, skipBillingSubmit } = arg;
            // =========================================================================
            ...
            if (skipBillingSubmit) {
                console.log('[Step] DEMO_SKIP_BILLING_SUBMIT: not clicking Post (demo / video mode).');
                return {
                    ok: true,
                    amount,
                    days,
                    verified: false,
                    demoSkippedSubmit: true,
                    warning: 'Demo mode: billing Post button not clicked'
                };
            }
            ...
            //  INJECTED LOGIC END
            // =========================================================================
        }, { data: requestData, sel, skipBillingSubmit });
